<template>
  <div class="ticket-workspace flex h-full">
    ...
    <div class="hidden md:flex w-[320px] flex-shrink-0 border-r border-slate-200 dark:border-slate-800">
      <TicketQueueColumn :selected-ticket-id="selectedTicketId" :project-key="projectKey" />
    </div>

    <!-- Right content area — full width on mobile, flexible on desktop -->
    <div class="flex-1 min-w-0 flex flex-col overflow-hidden">
      <router-view v-slot="{ Component: PageComponent }">
        <component :is="PageComponent" :selected-ticket-id="selectedTicketId" :project-key="projectKey" />
      </router-view>
    </div>
  </div>
</template>
...
const selectedTicketId = computed(() => route.params.issueKey as string | undefined);
